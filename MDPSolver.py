...
def valueIteration(env, reward, loss=None, tol=1e-6):
    V = np.zeros((env.n_states))
    Q = np.zeros((env.n_states, env.n_actions))
    #reward = env.get_reward_for_given_w(w)
    if loss is None:
        loss=np.zeros(len(reward))
    reward += loss
    ...
    while True:
        ...
        for a in range(env.n_actions):
            Q[:, a] = reward + env.gamma * env.T_sparse_list[a].dot(V)
...
def soft_valueIteration(env, reward, tol=1e-6):
    V = np.zeros((env.n_states))
    Q = np.zeros((env.n_states, env.n_actions))
    #reward = env.get_reward_for_given_w(w)
...
def adapted_soft_valueIteration(env, reward, variance, tol=1e-6):
    V = np.zeros((env.n_states))
    Q = np.zeros((env.n_states, env.n_actions))
    ...
    while True:
        ...
        for a in range(env.n_actions):
            Q[:, a] = reward + variance + env.gamma * env.T_sparse_list[a].dot(V)
...
def computeFeatureSVF_sampling_state(env, policy, num_episode, len_episode, init_state=None):
    ...
    for i in range(num_episode):
        episode, state_count_gamma = generateEpisode(env, policy, len_episode, init_state)
    ...
    return total_state_count_gamma/num_episode, feature_expectation/num_episode, episode_list, mu_list, sv_list


def computeFeatureSVF_sampling(env, policy, num_episode, len_episode):
    # Given a policy, return feature expectation and state-visitation frequency
    # Step1: Generate "m" trajectories of that policy (sampling)
    # Step2: Compute the feature expectation using Equation5 of the paper
    ...
    total_state_count_gamma = np.zeros(env.n_states)
    for i in range(num_episode):
        _, state_count_gamma = generateEpisode(env, policy, len_episode)
        total_state_count_gamma += state_count_gamma
    feature_expectation = env.get_state_feature_matrix().transpose().dot(total_state_count_gamma)
    # return both state visitation frequency and feature expectation
    return total_state_count_gamma/num_episode, feature_expectation/num_episode


def generateEpisode(env, policy, len_episode, init_state=None):
...
def computeFeatureSVF_bellmann_averaged(env, policy, num_iter=None):
    ...
    for i in range(num_iter):
        SV, feature_expectation = computeFeatureSVF_bellmann(env, policy)
        sv_list.append(SV)
        mu_list.append(feature_expectation)

    return np.mean(sv_list, axis=0), np.mean(mu_list, axis=0)
...
def computeFeatureSVF_bellmann(env, policy, tol=1e-6):
    ...
    return SV, feature_expectation

def computeValueFunction_bellmann_averaged(env, policy, reward, num_iter=None):
    ...
    V_list = []
    for i in range(num_iter):
        V_list.append(computeValueFunction_bellmann(env, policy, reward))
...
def computeValueFunction_bellmann(env, policy, reward, tol=1e-6):
    ...
    while True:
        iter += 1
        V_old = copy.deepcopy(V)
        V[:] = reward + env.gamma * T_pi_sparse.dot(V)
        if abs(np.linalg.norm(V - V_old, np.inf)) < tol: 
            #converged
            break
    return V
