...
def run_experiment(args):
    ...
    if store:
        plt.savefig(
            Path("plots") / "object_world_environment" / f"n{n_trajectories}_r{run}_real_rewards.jpg",
            format="jpg",
        )
    ...
    learned_reward_grid_expectation = agent_expectation.rewards[-1].reshape(env.grid_size, env.grid_size)
    lrge_min = learned_reward_grid_expectation.min()
    lrge_max = learned_reward_grid_expectation.max()
    learned_reward_grid_expectation = ((learned_reward_grid_expectation - lrge_min) / (lrge_max - lrge_min)) * 2 -1
    ...
    wandb.log(
        {
            ...
            "reward_grid_diff_expectation": learned_reward_grid_expectation - reward_matrix,
            "reward_grid_diff_norm_expectation": np.linalg.norm(learned_reward_grid_expectation - reward_matrix)
        }
    )
    ...
    learned_reward_grid_variance = agent_variance.rewards[-1].reshape(env.grid_size, env.grid_size)
    lrgv_min = learned_reward_grid_variance.min()
    lrgv_max = learned_reward_grid_variance.max()
    learned_reward_grid_variance = ((learned_reward_grid_variance - lrgv_min) / (lrgv_max - lrgv_min)) * 2 -1
    ...
    wandb.log(
        {
            ...
            "reward_grid_diff_norm_variance": np.linalg.norm(learned_reward_grid_variance - reward_matrix)
